...
class FLowFEncoder(nn.Module):
    def __init__(self):
        super().__init__()
        self.segFormer = FLowFSegFormer(img_size=64,
                                        patch_size=3,
                                        stride=2,
                                        in_chans=256,
                                        embed_dims=1024,
        ...
        self.pixel_shuffle = nn.PixelShuffle(upscale_factor = 2.0)
...
# This class is the modified version of the original SegFormer
# It has 1 OverlapPatchEmbed layer and 5 efficient attention blocks,
# which corresponds to only one stage in the original MixVisionTransformer class
class FLowFSegFormer(nn.Module):
    def __init__(
        self,
        img_size=224,
        patch_size=3,
        ...
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    # drop_path=dpr[cur + i], No drop out
...
# ---------------------------------------------------------------
# The following is the implementation of original SegFormer
# ---------------------------------------------------------------
class Mlp(nn.Module):
